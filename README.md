...
6. From here, you can run the command: `bash team300-travelea -Dplay.http.secret.key=QCYtAnfkaZiwrNwnxIlR6CTfG3gf90Latabg5241ABR5W1uDFNIk1 -Dplay.evolutions.db.default.autoApply=true -Dhttp.port=443`.
6. If you do not wish to run that command, you can create a file called `run` with the contents below:
	...
	./team300-travelea -Dplay.http.secret.key=QCYtAnfkaZiwrNwnxIlR6CTfG3gf90Latabg5241ABR5W1uDFNIk1 -Dplay.evolutions.db.default.autoApply=true -Dhttp.port=443
	```
    (Run the file by double clicking it and choosing "run in terminal" OR more simply run the command: `./run`)
8. Regardless of which method you choose, to populate the database, run the following command: `curl -X POST http://csse-s302g3.canterbury.ac.nz:443/api/populate`
9. Navigate to: `http://csse-s302g3.canterbury.ac.nz:443/`
10. Login with email: *admin@test.com*, password: *123*
