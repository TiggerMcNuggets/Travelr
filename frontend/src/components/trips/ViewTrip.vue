      ...
      <v-flex md3 pa-2>
        ...
        <v-form lazy-validation ref="form" v-model="isFormValid">
          ...
          <v-layout flex>
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn v-on:click="addTripNode(false)" icon v-on="on">
            ...
            <v-tooltip v-if="hasMissingDates && canDownloadTrip" bottom>
              <template v-slot:activator="{on}">
                <v-btn icon @click="downloadTrip()" flat small color="primary lighten-1" v-on="on">
                  <v-icon>calendar_today</v-icon>
                </v-btn>
              </template>
              <span>This calendar has missing destinations</span>
            </v-tooltip>
            <v-btn v-else-if="!canDownloadTrip" icon flat @click="downloadTrip()">
              <v-icon>calendar_today</v-icon>
            </v-btn>
          ...
          <v-timeline align-top dense>
            <draggable class="list-group" tag="ul" v-model="trip.trip.nodes">
              <transition-group type="transition" :name="!drag ? 'flip-list' : null">
                ...
                >
                  <v-timeline-item v-if="node.type.toLowerCase() === 'destination'">
                    <v-card class="v-timeline-destination-item-style">
                      <v-combobox
                  ...
                  <v-timeline-item v-else color="red">
                    <v-card>
                      <v-text-field
  ...
  methods: {
    ...
    changeNodeType(index) {
      const destinationType =
        this.trip.trip.nodes[index].type.toLowerCase() === "destination";
      this.trip.trip.nodes[index].type = destinationType
        ? "trip"
        : "destination";
    },
    ...
    toggleHiddenDestinations(parent) {
      ...
      while (
        i < destsLength &&
        this.trip.destinations[i].depth > parent.depth
      ) {
        this.$set(
          this.trip.destinations[i],
          "hidden",
          !this.trip.destinations[i].hidden
        );
        i = i + 1;
      }
    },
    ...
    noAdjacentIdenticalDestinations() {
      for (let i = 0; i < this.trip.trip.nodes.length - 1; i++) {
        if (
          this.trip.trip.nodes[i].type === "destination" &&
          this.trip.trip.nodes[i + 1].type === "destination"
        ) {
          if (
            this.trip.trip.nodes[i].destination.id ===
            this.trip.trip.nodes[i + 1].destination.id
          ) {
