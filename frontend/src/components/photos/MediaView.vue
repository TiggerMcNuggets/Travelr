<template>
  <v-dialog v-model="imageDialog" width="70%">
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-select style="width: 20%" v-model="value" :items="albums" label="Albums" multiple>
        ...
        <v-spacer></v-spacer>
        <v-switch v-model="makeProfileMedia" :label="`Made Profile Media`"></v-switch>
        <v-switch v-if="isPublic" disabled v-model="publicPhotoSwitch" :label="`Public Photo`"></v-switch>
      ...
      <v-card-actions>
        <v-btn color="primary" flat @click="closeMediaDialog">Close</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
